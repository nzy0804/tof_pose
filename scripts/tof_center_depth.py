...
DISPLAY_SCALE = 8
WINDOW_NAME = "tof_center_depth"
LOG_PREFIX = "[tof_center_depth]"
    ...
    def processor_thread() -> None:
        ...
        while not stop_event.is_set() or not frame_queue.empty():
            ...
            if depth.size != res_r * res_c:
                continue
            depth = depth.reshape((res_r, res_c)) - 35

            center_x = res_c // 2
            center_y = res_r // 2
            center_depth = int(depth[center_y, center_x])

            color_img = cv2.applyColorMap(depth, cv2.COLORMAP_MAGMA)
            cv2.drawMarker(
                color_img,
                (center_x, center_y),
                (255, 255, 255),
            ...
            if now - last_print >= 0.1:
                print(
                    f"\r{LOG_PREFIX} center=({center_x},{center_y}) depth={center_depth}   ",
                    end="",
                    flush=True,
                )
                last_print = now
            ...
            display_show = cv2.resize(
                color_img,
                (res_c * DISPLAY_SCALE, res_r * DISPLAY_SCALE),
